...
#General
input_dir=""
temp_dir="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/04_TempDir/"
output_dir="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/05_OutputDir/"
## Other settings which need to be defined somewhere
if 'snap' not in globals():
    snap="C:/Program Files/snap/bin/gpt"
if 'ErrorMessage' not in globals():
    ErrorMessage=[]

###SAIMON related
GPT_WQ_Graph="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/02_Scripting/WQ_Graph_SAI.xml"
GPT_SST_Graph="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/02_Scripting/SST_Graph_SAI.xml"
GPT_WQ_M_Graph="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/02_Scripting/Mosaic_WQ_Graph.xml"
GPT_SST_M_Graph="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/02_Scripting/Mosaic_SST_Graph.xml"
pFilenames=['Chl','WT','Tur','SST']
Legends=['C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/01_Ancillari/Legenda_CHL.txt',
         'C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/01_Ancillari/Legenda_TR.txt',
         'C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/01_Ancillari/Legenda_Turb_provv.txt',
         'C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/01_Ancillari/Legenda_SST.txt']
Mask_LandSea="C:/RheticusPktProcessors/WaterQuality/Modis_Saimon/01_Ancillari/Mask_Sea_thesprotia_AOI.tif"
...
def run(argsDict):
    ...
    # Clean Processor Temp data
    for tempf in os.listdir(temp_dir):
        if( os.path.isfile(os.path.join(temp_dir, tempf)) ):
            os.remove( os.path.join(temp_dir, tempf) )
    
    retcode = WQ_Chain(argsDict)
    
    # Archive output in "runDate" folder
    if not os.path.exists( os.path.join(output_dir, argsDict["runDate"]) ):
        os.makedirs( os.path.join(output_dir+argsDict["runDate"]) )
    else:
        for oldoutf in os.listdir( os.path.join(output_dir, argsDict["runDate"]) ):
            if( os.path.isfile(os.path.join(output_dir, argsDict["runDate"], oldoutf)) ):
                os.remove( os.path.join(output_dir, argsDict["runDate"], oldoutf) )
    for outf in os.listdir(output_dir):
        if( os.path.isfile(os.path.join(output_dir, outf)) ):
            os.rename( os.path.join(output_dir, outf), os.path.join(output_dir, argsDict["runDate"], outf) )
    
    print "\nWQ_Chain terminated: ", ErrorMessage, " (", retcode, ")"
    
    return { "returnCode": retcode,  "outPath": shared_path+argsDict["runDate"]+"\\" }
...
def WQ_Chain(argsDict):
    ...
    if (len(dateslist)==1):
        ...
        for d in range(1,len(dateslist)):
            ...
            dated_filename=filename[0]+str(norm_date.year)+norm_month[-2:]+norm_day[-2:]
                ...
                try:
                    ...
                    if lalegenda[0] != 0 or lalegenda[1] != 0:
                        ...
                        if (len(Re[0]) > 1):
                            ...
                            Be=None
                            if res == 1:
                                ErrorMessage.append("Error in creaing thematic RGB for "+output_dir+dated_filename+"_"+pFilenames[i]+"_Num.tif")
                        else:
                            ErrorMessage.append("Error in applying legend to "+output_dir+dated_filename+"_"+pFilenames[i]+"_Num.tif")
                    else:
                        ErrorMessage.append("Error in loading legend for "+output_dir+dated_filename+"_"+pFilenames[i]+"_Num.tif")
        data=None 
