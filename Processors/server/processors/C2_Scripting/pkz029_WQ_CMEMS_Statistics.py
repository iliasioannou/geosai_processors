...
if _platform == "linux" or _platform == "linux2":
    ...
    logging.basicConfig(filename=global_output_dir+'thisismystat.log',filemode='w',format='[CMEMS] %(asctime)s %(message)s',datefmt='%H:%M:%S',level=logging.DEBUG)
...
def P90_Mean_multiplefiles(filelista,tilesize,P90_outname,Mean_outname,seamask,land_value,nodata_value):
    ...
    for ilfile in filelista:
        ...
        if np.abs(ULx-geotrasf[0])>0.0000001 or np.abs(ULy-geotrasf[3])>0.0000001:
            logging.debug("[CMEMS_PROCESSORS] "+os.path.basename(ilfile)+" has different UL corner coordinates w.r.t. reference file "+os.path.basename(filelista[0])+" (removed)")
            filelista[filelista.index(ilfile)]=''
            ds=None
            continue
        if np.abs(PXx-geotrasf[1])>0.00001 or np.abs(PXy-geotrasf[5])>0.00001:
            logging.debug("[CMEMS_PROCESSORS] "+os.path.basename(ilfile)+" has different pixel size w.r.t. reference file "+os.path.basename(filelista[0])+" (removed)")
            filelista[filelista.index(ilfile)]=''
            ds=None
            continue
        if (SIZx != ds.RasterXSize) or (SIZy != ds.RasterYSize):
            logging.debug("[CMEMS_PROCESSORS] "+os.path.basename(ilfile)+" has different pixels w.r.t. reference file "+os.path.basename(filelista[0])+" (removed)")
            filelista[filelista.index(ilfile)]=''
            ds=None
            continue
        ds=None
        ...
        for i in range(0,SIZy,tilesize):
            ...
            for ilfile in filelista:
                ...
                try:
                    ...
                    logging.debug("[CMEMS_PROCESSORS] "+"Cannot read tile of"+os.path.basename(ilfile)+"("+e+")")
                    ds=None
                except AttributeError, e:
                    logging.debug("[CMEMS_PROCESSORS] "+"Cannot read tile of"+os.path.basename(ilfile)+"("+e+")")
                    ds=None
        ...
        if os.path.exists(seamask)==False:
            ...
            try:
                ...
                if (landcols != SIZy) or (landrows != SIZx):
                    logging.debug("[CMEMS_PROCESSORS] "+"Land mask ("+seamask+") raster size not compatible with products")
                else:
...
def WQ_Stats_CMEMS(WorkingDate,stat_type,AOI):
    ...
    if (erro==1) or os.path.exists(SMask_LandSea)==None:
        logging.debug("[CMEMS_PROCESSORS] Error in cutting sea mask "+Mask_LandSea[AOI]+" ! Sea mask will be not applied.")
        SMask_LandSea=''
    ...
    if (stat_type==0):
        ...
        for el in pProds:    
            ...
            if len(matches)>2:
                ...
                logging.debug("[CMEMS_PROCESSORS] "+"Not enough products ("+str(len(matches))+") to generate 10-days stats for "+el)
                erro=erro+1
                if not os.path.exists(dest_dir):
                    os.rmdir(dest_dir)
                continue
        ...
        if (WorkingDate[2] == 2):
            ...
            for el in pProds:
                ...
                if len(matches)>2:
                    ...
                    logging.debug("[CMEMS_PROCESSORS] "+"Not enough products ("+str(len(matches))+") to generate monthly stats for "+el)
                    erro=erro+1
                    continue
                    ...
                    if len(res[0])==0:
                        ...
                        if el == 'Chl':
                            ...
                            if lalegenda[0] != 0 or lalegenda[1] != 0:
                                ...
                                if (len(Re[0]) > 1):
                                    loggio=[]
                                    lres=RGB_as_input(res[0],Re,Ge,Be,dest_dir+prefix+el+'_P90_Thematic.tif',loggio)
                                    for lili in loggio: logging.debug("[CMEMS_PROCESSORS] "+lili)
                                    Re=None
                                    Ge=None
                                    Be=None
            ...
            logging.debug("[CMEMS_PROCESSORS] "+"The provided working date is not compatible with the monthly product")
            erro=erro+1
